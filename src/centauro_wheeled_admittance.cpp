#include "centauro_wheeled_admittance.h"
...
{
    return new XBot::Cartesian::CentauroWheeledAdmittance(model);
}

namespace XBot { namespace Cartesian {

CentauroWheeledAdmittance::CentauroWheeledAdmittance(ModelInterface::Ptr model):
    CartesianInterfaceImpl(model, __generate_tasks()),
    _logger(MatLogger::getLogger("/tmp/centauro_wheeled_admittance_log"))
{
...
bool CentauroWheeledAdmittance::setBaseLink(const std::string& ee_name, const std::string& new_base_link)
{
...
bool CentauroWheeledAdmittance::setControlMode(const std::string& ee_name, ControlType ctrl_type)
{
...
bool CentauroWheeledAdmittance::reset(double time)
{
...
bool CentauroWheeledAdmittance::update(double time, double period)
{
    bool success = true;
...
std::string CentauroWheeledAdmittance::get_parent(std::string link)
{
    return _model->getUrdf().getLink(link)->parent_joint->parent_link_name;
}

ProblemDescription CentauroWheeledAdmittance::__generate_tasks()
{
    AggregatedTask tasks;
    
    tasks.push_back( MakeInteraction("pelvis"  ,"world" ));
...
CentauroWheeledAdmittance::~CentauroWheeledAdmittance()
{
    _logger->flush();
}
...
void CustomRelativeCartesian::_update(const Eigen::VectorXd& x)
{
    ...
    _A -= (_Jbase.topRows(3) -   ::XBot::Utils::skewSymmetricMatrix(distal_pos - base_pos)*_Jbase.bottomRows(3));
